...
async function init() {
    try {
        const [tripsRes, eventsRes] = await Promise.all([
            fetch('data/trips.json?v=32'),
            fetch('data/events.json?v=32')
        ]);
...
function renderTimelineView(trips) {
    ...
    for (const year of years) {
        ...
        for (const trip of yearTrips) {
            ...
            const home = isHomeTrip(trip);
            const homeCls = home ? ' home-trip' : '';
            const duration = tripDurationText(range.start, range.end);

            html += '<div class="timeline-trip' + homeCls + '" id="tl-trip-' + ti + '">';
            html += '<div class="timeline-trip-header" onclick="document.getElementById(\'tl-trip-' + ti + '\').classList.toggle(\'expanded\')">';
            html += '<span class="expand-icon">\u25B6</span>';
            html += '<span class="timeline-trip-title">' + esc(trip.TripName) + '</span>';
            html += '<span class="timeline-trip-dates">' + fmtShort(range.start) + ' to ' + fmtShort(range.end);
            if (duration) html += ' <span class="trip-duration">(' + duration + ')</span>';
            html += '</span>';
            html += '</div>';
...
function getMapFilteredData() {
    ...
    let filteredTrips = tripsData.filter(t => !isHomeTrip(t));

    if (yearVal !== 'all') {
        filteredTrips = filteredTrips.filter(t => String(getTripYear(t)) === yearVal);
    }
    if (tripVal !== 'all') {
        filteredTrips = filteredTrips.filter(t => t.TripId === tripVal);
    }
    if (shipVal !== 'all') {
        filteredTrips = filteredTrips.filter(t =>
            (t.Segments || []).some(s => s.Ship === shipVal)
        );
    }
    if (typeVal !== 'all') {
        // Filter segments within trips to only show matching type
        filteredTrips = filteredTrips.filter(t =>
            (t.Segments || []).some(s => s.SegmentType === typeVal)
        );
    }

    // Filter events to match filtered trips
    const tripIds = new Set(filteredTrips.map(t => t.TripId));
    const filteredEvents = eventsData.filter(e => tripIds.has(e.TripId));

    return { trips: filteredTrips, events: filteredEvents };
}
...
function populateMapFilters() {
    ...
    for (const trip of tripsData) {
        ...
        const yr = getTripYear(trip);
        if (yr < 9999) years.add(yr);
        for (const seg of trip.Segments || []) {
            if (seg.Ship) ships.add(seg.Ship);
        }
    }

    const shipSelect = document.getElementById('map-ship-filter');
    ...
    const tripSelect = document.getElementById('map-trip-filter');
    tripSelect.innerHTML = '<option value="all">All Trips</option>';
    const sortedTrips = sortTripsDescending(tripsData.filter(t => !isHomeTrip(t)));
    for (const trip of sortedTrips) {
        const opt = document.createElement('option');
        opt.value = trip.TripId;
        opt.textContent = trip.TripName;
        tripSelect.appendChild(opt);
    }
}

function applyMapFilters() {
    const { trips, events } = getMapFilteredData();
    refreshMap(trips, events);
    ...
    if (hud) {
        ...
        const locs = new Set();
        for (const seg of allSegs) {
            const f = getSegFrom(seg);
            const t = getSegTo(seg);
            if (f) locs.add(f);
            if (t) locs.add(t);
        }
        hud.innerHTML = '<span class="hud-stat">' + trips.length + ' trips</span>'
            + '<span class="hud-stat">' + locs.size + ' locations</span>'
            + '<span class="hud-stat">' + countries.size + ' countries</span>';
    }
}
...
function render() {
    ...
    else if (currentView === 'globe') applyMapFilters();
}
...
async function init() {
    ...
    // Populate year filter for table/timeline
    ...
    // Map filter listeners
    ['map-ship-filter', 'map-year-filter', 'map-trip-filter', 'map-type-filter'].forEach(id => {
    ...
    document.querySelectorAll('.nav-btn').forEach(btn => {
        btn.onclick = function() {
            ...
            // Toggle filter bars
            document.getElementById('map-filters').style.display = currentView === 'globe' ? 'flex' : 'none';
            document.getElementById('filters').style.display = (currentView === 'table' || currentView === 'timeline' || currentView === 'gaps') ? 'flex' : 'none';
    ...
    // Table/timeline filter listeners
    document.getElementById('search-input').addEventListener('input', render);
    document.getElementById('year-filter').addEventListener('change', render);
    document.getElementById('type-filter').addEventListener('change', render);
    document.getElementById('source-filter').addEventListener('change', render);
