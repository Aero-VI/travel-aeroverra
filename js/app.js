...
function getConnectionOnlyCountries() {
    ...
    const globalConnectionAirports = new Set(); // airport codes that are connections somewhere
    ...
    // A country is "connection-only" if every flight touching it uses a connection airport
    // AND there are no hotels, cruises, trains, or bus stops in that country
    ...
    for (const trip of tripsData) {
        for (const seg of trip.Segments || []) {
            ...
            if (st === 'Accommodation' || st === 'Hotel') {
                ...
                // If the airport is NOT a known connection airport, this is a real visit
                if (dep.CountryCode) {
...
function renderCountriesView() {
    ...
    const connectionOnly = getConnectionOnlyCountries();
    const allCodesUnfiltered = Object.keys(countryData).sort((a, b) => {
        const va = countryData[a].visits, vb = countryData[b].visits;
        if (va !== vb) return vb - va;
        return countryName(a).localeCompare(countryName(b));
    });
    const allCodes = hideConnections ? allCodesUnfiltered.filter(c => !connectionOnly.has(c)) : allCodesUnfiltered;
    ...
    // Connection toggle
    if (connectionOnly.size > 0) {
        html += '<div class="countries-toggle">';
        html += '<label class="ct-label"><input type="checkbox" id="connections-toggle" ' + (hideConnections ? '' : 'checked') + '> Show connection-only countries (' + connectionOnly.size + ')</label>';
        html += '</div>';
    }
    ...
    const connToggle = document.getElementById('connections-toggle');
    if (connToggle) {
        connToggle.onchange = function() {
            hideConnections = !this.checked;
            renderCountriesView();
        };
    }
}
...
function renderCountryCards(codes, countryData, sortMode) {
    ...
    for (const code of codes) {
        ...
        const isConn = typeof getConnectionOnlyCountries === 'function' && getConnectionOnlyCountries().has(code);
        html += '<div class="country-card' + (isConn ? ' connection-only' : '') + '" onclick="this.classList.toggle(\'expanded\')">';
        ...
        if (isConn) html += '<span class="cc-connection-badge">Connection</span>';
        html += '</div>';
